...
public class HorizontalAssemblingStrategy extends ListAssemblingStrategy
{
	...
	public BufferedImage assemble(ArrayList<BufferedImage> elements, int cellWidth, int cellHeight, Theme theme, boolean inner)
	{
		...
		int width = cellWidth * elements.size();
		width += theme.internalBorder * Math.max(0,elements.size()-1);
		width += 2*border;
		width = Math.max(1, width);
		int height = cellHeight + 2*border;
		...
		{
			int x = (cellWidth + theme.internalBorder) * i + border;
			int y = border;
	...
	{
		int border = theme.getExternalBorder(inner);
		int width = cellWidth * count + theme.internalBorder * (count-1) + 2*border;
		int height = cellHeight + 2*border;
